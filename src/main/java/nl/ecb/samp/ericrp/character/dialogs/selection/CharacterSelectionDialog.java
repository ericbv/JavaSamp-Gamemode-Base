...
public class CharacterSelectionDialog extends AbstractListDialog {
	...
	public CharacterSelectionDialog(Player player, Shoebill shoebill,
		...
		this.player = player;
		this.con = con;
		caption =("Character Mangement");
	}

	public void show() {
		try {
			List<Character> Characters = AccountStore.getInstance()
					.getAccount(player).getCharacters();
			for (final Character c : Characters) {
				DateFormat df = new SimpleDateFormat("dd-MM-yyyy");
				dialogListItems.add(new DialogListItem("- "
						+ c.getCharacterName() + " " + c.getHisOrHerGender()
		...
		dialogListItems.add(new DialogListItem("Create Character") {
			@Override
			public void onItemSelect() {
				new CharacterCreationManager(player, shoebill,
						rootEventManager, con);
			}

		});
		dialogListItems.add(new DialogListItem("Delete Character") {
			@Override
			public void onItemSelect() {
				new CharacterDeletionManager(player, shoebill,
						rootEventManager, con);
